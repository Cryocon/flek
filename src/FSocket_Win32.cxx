...
int win_socket::open(int listen_queue_size)
{
    unsigned short port;
    char* p;
...
int win_socket::connect(int max_attempts, time_t timeout)
{
...
int win_socket::read(void* buf, size_t size)
{ 
    if (state != ss_open) { 
	errcode = not_opened;
	return 0;
...
int win_socket::write(void const* buf, size_t size)
{ 
    if (state != ss_open) { 
	errcode = not_opened;
	return 0;
...
int local_win_socket::read(void* buf, size_t size)
{
    char* dst = (char*)buf;
    Error = ok;
...
int local_win_socket::write(const void* buf, size_t size)
{
    char* src = (char*)buf;
    Error = ok;
    while (size > 0 && state == ss_open) {	
...
int local_win_socket::cancel_accept() 
{
    state = ss_shutdown;
    SetEvent(Signal[RD]);
    SetEvent(Signal[RTT]);
...
int local_win_socket::connect(int max_attempts, time_t timeout)
{
    char buf[MAX_ADDRESS_LEN];
    int  rc, i, error_code;
    HANDLE h[2];
